...
module.exports = function(app, BASE_PATH){
    console.log("Registering sportsAPI (v2): sportsCenters.");
    var path = "";
 ...
 // Carga datos iniciales 
    ...
    app.get(path, (req,res) =>{
        ...
        if (typeof offset === 'undefined') {
         ...
         sportsCenters.find(myquery, { projection: { _id: 0 } }).skip(offset).limit(limit).toArray((err, sportscentersArray) => {
            ...
            var filteredCenters = sportscentersArray;
            if (!isNaN(fromYear) && typeof fromYear !== 'undefined' && typeof fromYear !== 'null') {
                fromYear = parseInt(fromYear, 10);
                filteredCenters = sportscentersArray
                .filter((comp) => {
                    return (comp.startingyear >= fromYear);
                });
            }
            if (toYear>=fromYear && !isNaN(toYear) && typeof toYear !== 'undefined' && typeof toYear !== 'null') {
                toYear = parseInt(toYear, 10);
                filteredCenters = filteredCenters
                .filter((comp) => {
                    return (comp.startingyear <= toYear);
                });
            }
    ...
    console.log("sports-centers/ (v2) registered");
    }
